...
void WindowCapture::open(int window_handle)
{
    ...
    m_frame_pool = winrt::Direct3D11CaptureFramePool::CreateFreeThreaded(
        m_device,
        m_pixel_format,
        1,
        m_gc_item.Size());
    ...
    m_frame_pool.FrameArrived([this](auto& framePool, auto&)
        {
            m_frame = framePool.TryGetNextFrame();
    m_capture_event.SetEvent();  // Complete the operation
        });
...
winrt::com_ptr<ID3D11Texture2D> WindowCapture::get_texture()
{
    ...
    auto texture = utils::GetDXGIInterfaceFromObject<ID3D11Texture2D>(m_frame.Surface());
